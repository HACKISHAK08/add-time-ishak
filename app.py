...
def remove_friend():
    token = request.args.get('token')  # قراءة التوكن من query parameters
    target_id = request.args.get('id')  # قراءة الـ ID من query parameters
    ...
    if response.status_code == 200:
        ...
        return jsonify({"error": "فشل حذف الصديق!"}), 500
